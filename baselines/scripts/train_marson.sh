...
if [ "$DATASET_NAME" = "replogle" ]; then
    ...
    CONTROL_PERT="NTC"
    FOLD_NAME="fold${FOLD_ID}"
    # Marson adata has only .X (no obsm). Use null so cell_load uses raw .X.
    EMBED_KEY=null
    # Timepoint covariate (experimental_perturbation_time_point) has vocab size 3; + appends new key under struct config
    MODEL_EXTRA_ARGS="+model.kwargs.n_time_points=3"
fi
...
# Use a temp dir on the same filesystem as outputs to avoid "Invalid cross-device link" when saving checkpoints
...
if [ "$MODEL_NAME" = "lrlm" ]; then
    ...
    python -m state_sets_reproduce.train \
        data.kwargs.toml_config_path=$DATA_TOML_PATH \
        data.kwargs.embed_key=${EMBED_KEY:-X_hvg} \
        data.kwargs.basal_mapping_strategy=random \
        data.kwargs.output_space=gene \
        data.kwargs.num_workers=24 \
        data.kwargs.batch_col=${BATCH_COL} \
