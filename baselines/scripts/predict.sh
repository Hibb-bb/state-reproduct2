...
# Add local cell-load repo to PYTHONPATH
...
if [ "$DATASET_NAME" = "replogle" ]; then
    ...
    if [ -z "$TOML_OVERRIDE" ]; then
        TOML_OVERRIDE="$BASELINES_DIR/marson_gen.toml"
    fi
fi
...
TOML_ARG=""
if [ -n "$TOML_OVERRIDE" ]; then
    TOML_ARG="--toml_config_path ${TOML_OVERRIDE}"
    echo "Using toml override: $TOML_OVERRIDE"
fi
